...
*Setup instructions for a gitian build of Exsolution using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Exsolution
...
- [Building Exsolution](#building-exsolution)
...
Clone the git repositories for exsolution and gitian and then checkout the exsolution version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/exsolution/ext-wallet.git
cd exsolution
...
Follow the instructions in [doc/release-process.md](release-process.md) in the exsolution repository
...
To build Exsolution (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the exsolution repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/exsolution/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/exsolution/ext-wallet
...
./bin/gbuild --commit exsolution=${COMMIT} --url exsolution=${URL} ../exsolution/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit exsolution=${COMMIT} --url exsolution=${URL} ../exsolution/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit exsolution=${COMMIT} --url exsolution=${URL} ../exsolution/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/exsolution-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/exsolution-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/exsolution-build.assert
```
