...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ExsolutionMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ExsolutionMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ExsolutionMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("exsolution-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ExsolutionMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
