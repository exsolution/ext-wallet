...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::EXT)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("EXT")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeExsolutionAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeExsolutionAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeExsolutionAmount = strAnonymizeExsolutionAmount.remove(strAnonymizeExsolutionAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeExsolutionAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeExsolutionAmount * COIN) nMaxToAnonymize = nAnonymizeExsolutionAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeExsolutionAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeExsolutionAmount));
        strAnonymizeExsolutionAmount = strAnonymizeExsolutionAmount.remove(strAnonymizeExsolutionAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeExsolutionAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void Coinmix::toggleDarksend()
{
    ...
    if (!fEnableDarksend) {
        ...
        if (nAnonymizeExsolutionAmount == 0) {
